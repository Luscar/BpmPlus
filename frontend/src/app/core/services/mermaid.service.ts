...
import { DefinitionProcessus, NoeudProcessus, TypeNoeud } from '../models/definition.model';
...
export class MermaidService {
  ...
  private nodeShape(type: TypeNoeud, label: string): string {
    switch (type) {
  ...
  private nodeClass(type: TypeNoeud, estFinale: boolean): string {
  ...
  private fluxLabel(
    ...
    if (condition.type === 'ConditionQuery') {
      return condition.nomQuery ?? '';
    }
    return '';
  }
