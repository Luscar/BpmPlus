...
{
    public RepositoryDefinitionSqlite(string prefixe) : base(prefixe) { }
        ...
        DefinitionProcessus definition, IDbTransaction transaction, CancellationToken ct = default)
    {
        ...
        var derniereVersion = await Cn(transaction).QuerySingleOrDefaultAsync<int?>($"""
        ...
        string cle, IDbTransaction transaction, CancellationToken ct = default)
    {
        ...
        await Cn(transaction).ExecuteAsync($"""
            ...
            WHERE CLE = @Cle AND STATUT = 'Brouillon'
            """, new { Cle = cle, DatePublication = maintenant }, transaction);
    }
