...
{
    public RepositoryAttenteSignalSqlite(string prefixe) : base(prefixe) { }
    ...
    {
        var ids = await Cn(transaction).QueryAsync<long>($"""
    ...
    public async Task<IReadOnlyList<string>> ObtenirSignauxParInstanceAsync(
        long idInstance, IDbTransaction transaction, CancellationToken ct = default)
    {
        var signaux = await Cn(transaction).QueryAsync<string>($"""
